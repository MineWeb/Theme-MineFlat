...
<div class="container news">
    <div class="row">
      <div class="col-md-12">
        <div class="box-news">
            <h1><?= $news['News']['title'] ?> <span class="author"> Publié le <?= $Lang->date($news['News']['created']); ?> par <span class="blue"> <?= $news['News']['author'] ?></span></span></h1>
    ...
    <div class="container">
        ...
        <p>
            <?php if($Permissions->can('LIKE_NEWS')) { ?>
                ...
                <button id="<?= $news['News']['id'] ?>" type="button"  class="btn btn-primary btn-lg like<?= ($news['News']['liked']) ? ' active' : '' ?>"<?= (!$Permissions->can('LIKE_NEWS')) ? ' disabled' : '' ?> style="background-color: #ccc;color: #000;"><?= $news['News']['count_likes'] ?> <i class="fa fa-heart red"></i></button>
            <?php } else { ?>
    ...
    <div class="row">
        ...
        <?php if($Permissions->can('COMMENT_NEWS')) { ?>
          <div class="col-md-12 m-t-20">
            <a href="#" data-toggle="modal" data-target="#postcomment" class="btn btn-primary btn-lg pull-left m-radius"><?= $Lang->get('NEWS__COMMENT_TITLE') ?></a>
          </div>
        <?php } ?>
